        ...
        <div className='flex-1 flex flex-col lg:flex-row'>
          ...
          <div className='relative lg:w-[60%] flex-1 flex items-center'>
            <div className='flex flex-wrap gap-4 justify-center lg:justify-end lg:-mr-6'>
              {pets.map((pet, i) => {
                return (
                  <div key={i} className='cursor-pointer relative' onClick={() => {
                    getPetDetails(pet.id)
                    setPetIndex(i)
                  }}>
                    <div className={`bg-black/40 w-full h-full absolute rounded-full ${petIndex === i ? 'border-2 border-white' : 'bg-black/40'}`}></div>
